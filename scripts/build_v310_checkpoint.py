"""
构建 v3.1.0 物理预测器 checkpoint (3.1 线起点正式训练件)
================================================================
与 v3.0.3 同口径 (seed=42 / n_per_kind=48 / epochs=60 / patience=12 /
front / hybrid_weight=0): 主 checkpoint 不挂载 ActionPiece 码本, 主模型参数量
仍为 52191。ActionPiece tokenizer 为**推理时外挂**, 不参与主训练。

流程: 训练 -> OOD -> 校准 -> 独立集评估 -> save -> reload 验证 -> 批量一致性
      -> 2.7/2.8/2.9/3.0 特性快照 -> 3.1 ActionPiece 离线快照 (合成动作码本)。
落 checkpoints/predictor_v3.1.0.pt 与 benchmarks/results/training_v3.1.0.json。

analogy, not reproduction: ActionPiece 码本仅在合成动作向量上 k-means 拟合,
不碰真机动作 / 视频 / VLM。

用法:
    python3 scripts/build_v310_checkpoint.py --quick
    python3 scripts/build_v310_checkpoint.py          # 正式口径 (~100s, 2 线程)
"""
import argparse
import json
import os
...
def main():
    ...
    ckpt = "checkpoints/predictor_v3.1.0.pt"
    ...
    # ---- 3.1 ActionPiece 离线快照 (合成动作码本, 推理外挂) ----
    # 用独立集相邻状态差作为"动作代理"拟合 k-means 码本
    ...
    with open("benchmarks/results/training_v3.1.0.json", "w",
              encoding="utf-8") as f:
