"""
构建 v2.7.3 物理预测器 checkpoint (2.7 线最终正式发布件)
================================================================
与 v2.7.0 / v2.6.2 同口径 (seed=42 / n_per_kind=48 / epochs=60 / patience=12 / front /
hybrid_weight=0): 主 checkpoint **不挂载 hybrid**, 主模型参数量仍为 52191
(hybrid 为可选外挂)。2.7 新模块 (policy/online/active/lite/hierarchical/experiment)
均为推理时外挂, 不参与训练、不改变训练口径与主模型结构。

流程: 训练 -> OOD 拟合 -> 校准拟合 -> 独立集评估 -> save -> reload 验证 -> 批量一致性
      -> 2.7 特性离线 A/B 快照。落 checkpoints/predictor_v2.7.3.pt 与
      benchmarks/results/training_v2.7.3.json。

用法:
    python3 scripts/build_v273_checkpoint.py --quick   # 小规模快速复现
    python3 scripts/build_v273_checkpoint.py          # 正式口径 (~100s, 2 线程)
"""
import argparse
import json
import os
...
def main():
    ...
    ckpt = "checkpoints/predictor_v2.7.3.pt"
    ...
    assert meta["udos_version"] == "2.7.3", \
        f"checkpoint 版本 {meta['udos_version']} != 2.7.3"
    ...
    with open("benchmarks/results/training_v2.7.3.json", "w",
              encoding="utf-8") as f:
