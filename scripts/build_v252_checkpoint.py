"""
构建 v2.5.2 物理预测器 checkpoint (可复现, 2.5 线终点正式件)
================================================================
与 v2.5.0 同口径 (front 默认 + PAVA 校准 + conformal 区间 + OOD 拟合),
v2.5.2 为 2.5 线最终发布件。不新增可学参数 (参数量仍 ~52191)。

落 checkpoints/predictor_v2.5.2.pt 与 benchmarks/results/training_v2.5.2.json。

用法:
    python3 scripts/build_v252_checkpoint.py --quick
    python3 scripts/build_v252_checkpoint.py
"""
import argparse
import json
import os
...
def main():
    ...
    ckpt = "checkpoints/predictor_v2.5.2.pt"
    ...
    with open("benchmarks/results/training_v2.5.2.json", "w",
              encoding="utf-8") as f:
