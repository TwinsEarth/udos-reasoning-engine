"""
构建 v3.8.6 正式物理预测器 checkpoint (全域调度线首训件)
====================================================================
...
落 checkpoints/predictor_v3.8.6.pt 与 benchmarks/results/training_v3.8.6.json。

用法:
    python3 scripts/build_v386_checkpoint.py --quick
    python3 scripts/build_v386_checkpoint.py
"""
import argparse
import hashlib
import json
...
def main():
    ...
    ckpt = "checkpoints/predictor_v3.8.6.pt"
    ...
    # ---- 全域闭环零外挂自检 (v3.8.6 最终件) ----
    from udos.closed_loop import ClosedLoopOrchestrator  # noqa: E402
    cl = ClosedLoopOrchestrator(loaded, wm_horizon=2)
    cl_out = cl.step(build(7777, n_per_kind).X[:1])
    cl_sanity = {
        "command_shape": list(cl_out["command"].shape),
        "feedback_finite": cl_out["wm_feedback"]["feedback_l1_to_next"] >= 0,
        "window_updated": cl_out["perception"]["window_slid"],
        "zero_grad_state_dict_md5_unchanged": (sd0 == _md5(loaded)),
        "learned": False, "analogy_not_reproduction": True,
    }

    prev_mse = None
    for prev in ("checkpoints/predictor_v3.8.0.pt",
                 "checkpoints/predictor_v3.7.0.pt"):
