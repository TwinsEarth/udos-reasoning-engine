"""
构建 v3.0.0 物理预测器 checkpoint (3.0 线首个正式训练件)
================================================================
与 v2.9.0 / v2.8.0 同口径 (seed=42 / n_per_kind=48 / epochs=60 / patience=12 /
front / hybrid_weight=0): 主 checkpoint **不挂载 hybrid / PhysicalLoop /
retargeting / future_multimodal**, 主模型参数量仍为 52191。
FutureMultimodalHead 为推理时外挂多模态代理头 (纯前向只读), 不参与训练、
不改变训练口径与主模型结构。

流程: 训练 -> OOD 拟合 -> 校准拟合 -> 独立集评估 -> save -> reload 验证 -> 批量一致性
      -> 2.7 特性离线快照 -> 2.8 PhysicalLoop 离线快照 -> 2.9 retargeting 离线快照
      -> 3.0 future_multimodal 离线快照。
落 checkpoints/predictor_v3.0.0.pt 与 benchmarks/results/training_v3.0.0.json。

analogy, not reproduction: RGB/深度/mask 均为共享 latent 的低维线性投影代理,
不碰真实 RGBD 图像。

用法:
    python3 scripts/build_v300_checkpoint.py --quick   # 小规模快速复现
    python3 scripts/build_v300_checkpoint.py          # 正式口径 (~100s, 2 线程)
"""
import argparse
import json
import os
...
def main():
    ...
    ckpt = "checkpoints/predictor_v3.0.0.pt"
    ...
    assert meta["udos_version"] == "3.0.0", \
        f"checkpoint 版本 {meta['udos_version']} != 3.0.0"
    ...
    # ---- 3.0 future_multimodal 离线快照 (推理时外挂, 不改主模型) ----
    torch.manual_seed(0)
    mth = MultiTaskHead(loaded, latent_dim=32, enable=True)
    fmh = FutureMultimodalHead(32, horizon=4, rgb_dim=8, depth_dim=4,
                               mask_dim=4)
    mth.register_head("future_mm", fmh)
    mm_out = mth.forward(w1, scene_params=p1)["future_mm"]
    mm_bit = {k: list(v.shape) for k, v in mm_out.items()}
    mm_finite = all(torch.isfinite(v).all().item() for v in mm_out.values())
    # 默认关时主路径不变
    mth.enable = False
    assert mth.forward(w1, scene_params=p1) == {}
    features_v30 = {
        "modality_shapes": mm_bit,
        "all_finite": bool(mm_finite),
        "modalities": sorted(mm_out.keys()),
        "analogy_not_reproduction": True,
    }
    ...
    summary = {
        ...
        "features_v30": features_v30,
        "checkpoint": ckpt, "reload_consistent": True,
    }
    os.makedirs("benchmarks/results", exist_ok=True)
    with open("benchmarks/results/training_v3.0.0.json", "w",
              encoding="utf-8") as f:
