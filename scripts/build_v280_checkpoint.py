"""
构建 v2.8.0 物理预测器 checkpoint (2.8 线首个正式训练件)
================================================================
与 v2.7.3 / v2.7.0 同口径 (seed=42 / n_per_kind=48 / epochs=60 / patience=12 /
front / hybrid_weight=0): 主 checkpoint **不挂载 hybrid / PhysicalLoop**, 主模型
参数量仍为 52191。PhysicalLoopRunner 为推理时外挂编排层 (纯前向只读), 不参与
训练、不改变训练口径与主模型结构。

流程: 训练 -> OOD 拟合 -> 校准拟合 -> 独立集评估 -> save -> reload 验证 -> 批量一致性
      -> 2.7 特性离线 A/B 快照 -> 2.8 PhysicalLoop 离线快照。
落 checkpoints/predictor_v2.8.0.pt 与 benchmarks/results/training_v2.8.0.json。

analogy, not reproduction: PhysicalLoop 为受 PhysBrain 1.5 启发的轻量化类比编排层。

用法:
    python3 scripts/build_v280_checkpoint.py --quick   # 小规模快速复现
    python3 scripts/build_v280_checkpoint.py          # 正式口径 (~100s, 2 线程)
"""
import argparse
import json
import os
...
def main():
    ...
    assert meta["udos_version"] == "2.8.0", \
        f"checkpoint 版本 {meta['udos_version']} != 2.8.0"
    ...
    with open("benchmarks/results/training_v2.8.0.json", "w",
              encoding="utf-8") as f:
