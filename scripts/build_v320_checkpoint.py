"""
构建 v3.2.0 物理预测器 checkpoint (3.2 线起点正式训练件)
================================================================
与 v3.1.0 同口径 (seed=42 / n_per_kind=48 / epochs=60 / patience=12 /
front / hybrid_weight=0): 主 checkpoint 不挂载任何 3.2 线外挂, 主模型参数量
仍为 52191。SyntheticEgoAugmenter 为**训练时数据增强 opt-in**, 正式件仍用
原始数据 (增强不进正式训练口径, 见设计约束 2)。

流程: 训练 -> OOD -> 校准 -> 独立集评估 -> save -> reload 验证 -> 批量一致性
      -> 2.7..3.1 特性快照 -> 3.2 SyntheticEgoAugmenter 离线快照。
落 checkpoints/predictor_v3.2.0.pt 与 benchmarks/results/training_v3.2.0.json。

analogy, not reproduction: Ego360 增强仅在合成参数化状态序列上做视角/扰动/噪声/
时间缩放, 不碰真机视频 / 第一人称 RGB / VLM。

用法:
    python3 scripts/build_v320_checkpoint.py --quick
    python3 scripts/build_v320_checkpoint.py          # 正式口径 (~100s, 2 线程)
"""
import argparse
import json
import os
...
def main():
    ...
    ckpt = "checkpoints/predictor_v3.2.0.pt"
    ...
    assert meta["udos_version"] == __version__, \
        f"checkpoint 版本 {meta['udos_version']} != {__version__}"
    ...
    # ---- 3.2 Ego360 启发数据增强离线快照 (合成参数化状态, 推理外挂) ----
    ...
    with open("benchmarks/results/training_v3.2.0.json", "w",
              encoding="utf-8") as f:
