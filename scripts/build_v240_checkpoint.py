"""
构建 v2.4.0 物理预测器 checkpoint (可复现)
=========================================
在 v2.3.1 同口径训练(默认 front 权重=teacher-forcing 等价)之上新增 v2.4 OOD 能力:
- 训练集历史窗口 [N,W,RAW] 展平为特征, 拟合 DistributionDriftDetector
  (岭正则马氏距离 + 经验分位阈值);
- 在**独立校准集**上拟合保序置信校准 + 每步残差分位 (split-conformal);
- 在**独立测试集**上对照 校准前/后 ECE、区间覆盖率, 并验证 OOD 检测器:
  ID 样本低马氏距离、放大偏移样本高马氏距离且判为 OOD;
- 落 checkpoints/predictor_v2.4.0.pt (校准器+OOD 检测器随件持久化) 与
  benchmarks/results/training_v2.4.0.json, 多维校验保存/重载一致。

用法:
    python3 scripts/build_v240_checkpoint.py --quick
    python3 scripts/build_v240_checkpoint.py
"""
import argparse
import json
import os
...
def main():
    ...
    ckpt = "checkpoints/predictor_v2.4.0.pt"
    ...
    with open("benchmarks/results/training_v2.4.0.json", "w",
              encoding="utf-8") as f:
