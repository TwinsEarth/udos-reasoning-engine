"""
构建 v2.6.2 物理预测器 checkpoint (最终正式发布件, 2.6 线终点)
================================================================
与 v2.6.0 同口径 (seed=42 / n_per_kind=48 / epochs=60 / patience=12 / front /
hybrid_weight=0): 主 checkpoint **不挂载 hybrid**, 主模型参数量仍为 52191
(hybrid 为可选外挂)。在 v2.6.1 边缘加固后的代码上重建正式件。

流程: 训练 -> OOD 拟合 -> 校准拟合 -> 独立集评估 -> save -> reload 验证 -> 批量一致性自检。
落 checkpoints/predictor_v2.6.2.pt 与 benchmarks/results/training_v2.6.2.json。

用法:
    python3 scripts/build_v262_checkpoint.py --quick   # 小规模快速复现
    python3 scripts/build_v262_checkpoint.py          # 正式口径 (~100s, 2 线程)
"""
import argparse
import json
import os
...
def main():
    ...
    # 主 checkpoint 不挂 hybrid (hybrid_weight=0 默认关, 与 v2.6.0 逐位同口径)
    ...
    ckpt = "checkpoints/predictor_v2.6.2.pt"
    ...
    assert meta["udos_version"] == "2.6.2", \
        f"checkpoint 版本 {meta['udos_version']} != 2.6.2"
    ...
    with open("benchmarks/results/training_v2.6.2.json", "w",
              encoding="utf-8") as f:
