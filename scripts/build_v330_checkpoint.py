"""
构建 v3.3.0 物理预测器 checkpoint (3.3 线起点正式训练件)
================================================================
与 v3.2.0 同口径 (seed=42 / n_per_kind=48 / epochs=60 / patience=12 /
front / hybrid_weight=0)。**关键: 默认 CTM 架构配置 = 旧架构**
(residual=False / act_norm=False / init_mode="legacy"), 主 checkpoint 不挂载任何
3.3 线架构改进, 主模型参数量仍为 52191, 与 v3.2.0 逐位同合同。

3.3.0 架构精炼 (残差连接 / LayerNorm / 可配置初始化) 全部 opt-in, 默认关;
本正式件只记录其离线可用性快照, 不改变训练口径。

流程: 训练 -> OOD -> 校准 -> 独立集评估 -> save -> reload 验证 -> 批量一致性
      -> 2.7..3.2 特性快照 -> 3.3 架构精炼离线快照。
落 checkpoints/predictor_v3.3.0.pt 与 benchmarks/results/training_v3.3.0.json。

analogy, not reproduction: 残差/LayerNorm/初始化改进为通用深度学习算子,
不涉及任何外部预训练权重或真机。

用法:
    python3 scripts/build_v330_checkpoint.py --quick
    python3 scripts/build_v330_checkpoint.py          # 正式口径 (~100s, 2 线程)
"""
import argparse
import json
import os
...
from udos.ctm_engine import CTMConfig, CTMPhysicsEngine  # noqa: E402
...
def small_cfg(**overrides):
    # 默认即旧架构: residual=False / act_norm=False / init_mode="legacy"
...
def main():
    ...
    ckpt = "checkpoints/predictor_v3.3.0.pt"
    ...
    # ---- 3.1 ActionPiece 离线快照 ----
    deltas = ind_te.X[:, 1:, :] - ind_te.X[:, :-1, :]
    synth_actions = deltas.reshape(-1, deltas.size(-1))
    ap_tok = ActionPieceTokenizer(action_dim=6, codebook_size=16,
                                  init="kmeans++", seed=42).fit(synth_actions)
    ids = ap_tok.encode(synth_actions)
    features_v31 = {
        "action_dim": 6, "codebook_size": 16,
        "roundtrip_mse": round(ap_tok.roundtrip_error(synth_actions), 6),
        "utilization": round(ap_tok.utilization(), 4),
        "n_distinct_tokens": int(torch.unique(ids).numel()),
        "inertia_final": ap_tok.inertia_history_[-1],
        "analogy_not_reproduction": True,
    }

    # ---- 3.2 Ego360 启发数据增强离线快照 ----
    ego_aug = SyntheticEgoAugmenter(view_rotate_deg=20.0,
                                    view_translate_xyz=(0.1, -0.1, 0.0),
                                    traj_perturb=0.02, noise_sigma=0.01,
                                    time_scale=1.05, seed=42)
    Xa, Ya = ego_aug.augment_pair(ind_te.X[:32], ind_te.Y[:32])
    pure = SyntheticEgoAugmenter(view_rotate_deg=20.0,
                                 view_translate_xyz=(0.1, -0.1, 0.0))
    Xp, Yp = pure.augment_pair(ind_te.X[:32], ind_te.Y[:32])
    invert_err = (pure.reverse_view(Xp) - ind_te.X[:32]).abs().max().item()
    features_v32_ego = {
        "shapes_X": list(Xa.shape), "shapes_Y": list(Ya.shape),
        "all_finite": bool(torch.isfinite(Xa).all()
                           and torch.isfinite(Ya).all()),
        "view_roundtrip_max_abs_err": round(invert_err, 6),
        "augment_mean_deviation": round(
            float((Xa - ind_te.X[:32]).abs().mean()), 6),
        "config": ego_aug.describe(),
        "analogy_not_reproduction": True,
        "formal_trained_on_raw": True,
    }

    # ---- 3.3.0 架构精炼离线快照 (opt-in 默认关, 正式件=旧架构) ----
    torch.manual_seed(42)
    legacy_model = CTMPhysicsEngine(small_cfg())
    torch.manual_seed(42)
    optin_model = CTMPhysicsEngine(small_cfg(residual=True, act_norm=True,
                                             init_mode="he"))
    n_legacy = sum(p.numel() for p in legacy_model.parameters())
    n_optin = sum(p.numel() for p in optin_model.parameters())
    arch_x = ind_te.X[:4]
    enc = loaded.obs_encoder(arch_x)
    p_legacy, *_ = legacy_model.eval()(enc)
    p_optin, *_ = optin_model.eval()(enc)
    features_v33_arch = {
        "default_is_legacy_architecture": True,
        "n_params_legacy_engine": int(n_legacy),
        "n_params_optin_engine": int(n_optin),
        "optin_adds_act_norm_params": bool(n_optin > n_legacy),
        "optin_output_differs_from_legacy": bool(
            not torch.allclose(p_legacy, p_optin, atol=1e-6)),
        "optin_output_finite": bool(torch.isfinite(p_optin).all()),
        "analogy_not_reproduction": True,
    }
    ...
    summary = {
        ...
        "features_v31_actionpiece": features_v31,
        "features_v32_ego": features_v32_ego,
        "features_v33_arch": features_v33_arch,
        "new_features_offline_ab": {
            ...
            "action_piece": "inference add-on opt-in, main CTM unchanged",
            "ego_augment": "training-time data aug opt-in, formal ckpt on raw data",
            "ctm_arch_residual_ln_init": "opt-in, formal ckpt uses legacy arch (default off)",
        },
        "checkpoint": ckpt, "reload_consistent": True,
    }
    os.makedirs("benchmarks/results", exist_ok=True)
    with open("benchmarks/results/training_v3.3.0.json", "w",
              encoding="utf-8") as f:
